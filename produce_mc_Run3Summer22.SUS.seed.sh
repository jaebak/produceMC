...
echo "----GEN----"
# https://cms-pdmv-prod.web.cern.ch/mcm/public/restapi/requests/get_test/SUS-Run3Summer22GS-00055
...
cmsDriver.py Configuration/GenProduction/python/$Fragment_filename --era Run3 --customise Configuration/DataProcessing/Utils.addMonitoring --beamspot Realistic25ns13p6TeVEarly2022Collision --step GEN,SIM --geometry DB:Extended --conditions 124X_mcRun3_2022_realistic_v12 --customise_commands process.source.numberEventsInLuminosityBlock="cms.untracked.uint32(200) \n from IOMC.RandomEngine.RandomServiceHelper import RandomNumberServiceHelper ; randSvc = RandomNumberServiceHelper(process.RandomNumberGeneratorService) ; randSvc.resetSeeds(${SEED})" --datatier GEN-SIM --eventcontent RAWSIM --python_filename "$TAG"__LHE__cfg.py --fileout file:\$Output_filename -n $NEVENTS --no_exec --mc

echo "Run cmssw with configuration file"
cmsRun "$TAG"__LHE__cfg.py

echo "----DIGIPREMIX----"
# https://cms-pdmv-prod.web.cern.ch/mcm/public/restapi/requests/get_test/SUS-Run3Summer22DRPremix-00133
echo "Setting up CMSSW"
export SCRAM_ARCH=el8_amd64_gcc10
source /cvmfs/cms.cern.ch/cmsset_default.sh
...
cmsDriver.py  --era Run3 --customise Configuration/DataProcessing/Utils.addMonitoring --procModifiers siPixelQualityRawToDigi --step RAW2DIGI,L1Reco,RECO,RECOSIM --geometry DB:Extended --conditions 124X_mcRun3_2022_realistic_v12 --datatier AODSIM --eventcontent AODSIM --python_filename "$TAG"__AOD__cfg.py --fileout file:\$Output_filename --filein file:\$Input_filename -n -1 --no_exec --mc
...
echo "----MiniAODv4----"
# https://cms-pdmv-prod.web.cern.ch/mcm/public/restapi/requests/get_test/SUS-Run3Summer22MiniAODv4-00151
echo "Setting up CMSSW"
...
echo "----NanoAODv12----"
# https://cms-pdmv-prod.web.cern.ch/mcm/public/restapi/requests/get_test/SUS-Run3Summer22NanoAODv12-00048
echo "Setting up CMSSW"
export SCRAM_ARCH=el8_amd64_gcc11
source /cvmfs/cms.cern.ch/cmsset_default.sh
if [ -r CMSSW_13_0_23/src ] ; then
...
cmsDriver.py  --scenario pp --era Run3 --customise Configuration/DataProcessing/Utils.addMonitoring --step NANO --conditions 130X_mcRun3_2022_realistic_v5 --datatier NANOAODSIM --eventcontent NANOAODSIM --python_filename "$TAG"__NanoAODv12__cfg.py --fileout file:\$Output_filename --filein file:\$Input_filename -n -1 --no_exec --mc
